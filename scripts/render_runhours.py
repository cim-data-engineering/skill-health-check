...
themselves are the render contract summarised in that reference's Display
section and must not drift.
...
RIGHT_COL       = 250   # annotation column; sized for the longest annotation a
                        # 24/7 unit produces ("HH:MM-HH:MM · 24h  OOH 12h  +wknd 24h")
...
LEGEND_SWATCH   = 14
LEGEND_H        = 18
LEGEND_LABEL_PAD = 6    # swatch-to-label gap
LEGEND_ITEM_GAP = 28    # gap between one legend item and the next
GLYPH_W_RATIO   = 0.55  # mean glyph width as a fraction of font size; used only
                        # to advance the legend cursor, since SVG cannot measure text
...
def render(agg):
    ...
    for colour, label in items:
        ...
        p.append(f'<text x="{lx + LEGEND_SWATCH + LEGEND_LABEL_PAD}" y="{ty:.1f}" '
                 f'font-size="{FS_LEGEND}" fill="{C_TEXT}">{esc(label)}</text>')
        lx += (LEGEND_SWATCH + LEGEND_LABEL_PAD
               + len(label) * FS_LEGEND * GLYPH_W_RATIO + LEGEND_ITEM_GAP)
...
def main(argv):
    ...
    try:
        ...
        sys.exit(f"could not write SVG {argv[2]}: {exc}")
