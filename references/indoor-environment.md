...
Print the key under the table: `Excellent ≥92 · Good ≥85 · Average ≥75 · Poor <75`.

- `score` comes back as a raw `0.0–1.0` fraction — multiply by 100 and print to **1dp**. Format on the raw value, not the rounded one.
- The cell holds the number and nothing else — `93.4`, coloured. In the markdown fallback it is the bare number and the key carries the thresholds.
...
Band heatmap: one row per level, months as columns, each score cell filled with its band, the numeral still legible on the fill. Zones and Chg flank it uncoloured.
...
- Chg is signed with a glyph: `▲ +1.4`, `▼ -0.8`, `– 0.0`. Leave it uncoloured — it is a movement, not a score, and the bands do not apply to it.

## Links

Row label, over the full window — `summary_ts` is the first day of the current month:
...
Score cell, the same URL scoped to that month alone — `summary_ts` and `start_date` its first day, `end_date` the first day of the month after.
